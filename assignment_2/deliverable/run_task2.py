...
if __name__ == '__main__':
    ...
    # for example
    model_task2 = load_model('./deliverable/nn_task2.h5')

    # Predict on the given samples
    y_pred_task2 = model_task2.predict(x_test)

    matrix = confusion_matrix(y_test.argmax(axis=1), y_pred_task2.argmax(axis=1))
    plot_confusion_matrix(matrix, labels)

    # Evaluate the missclassification error on the test set
    assert y_test.shape == y_pred_task2.shape
    ca = CategoricalAccuracy()
    ca.update_state(y_test, y_pred_task2)
    acc2 = ca.result().numpy()
